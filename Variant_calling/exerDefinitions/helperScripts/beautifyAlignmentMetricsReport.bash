...
grep -A 10 "## METRICS CLASS" $1 \
| sed 1d \
| awk 'BEGIN{OFS="\t"; FS="\t";};{for( i=1 ; i<=NF ; i++ ){lines[i]= lines[i] "\t" $i;}};END{for( i=1 ; i<=length(lines) ; i++ ){print lines[i]}}' \
| paste - `dirname $0`/metricsDefs_alignment.txt | \
awk 'BEGIN{OFS="\t"; FS="\t";}; \
{if($2 == $6){print $2, $3, $4, $5 ,$7}else{print "Mismatch of definition at line "NR; print $2 "not matching " $6; print "Aborting"; exit}}; \
END{}'
